...
static void KeepDislocationsInsideSimulationVolume(DD_t* dd) {
...
void DD_ConstrainNodes(DD_t* dd) {
  // To deactivate this function, just return
  return;

  DD_KeepDislocationsOutsideInclusions(dd);

  // To keep the nodal positions within the simulation volume
  KeepDislocationsInsideSimulationVolume(dd);
}
...
static double AdaptiveElementLengthControl(int iElement, DD_t *dd) {
  ...
  max_cosine_angle =
      fmin(max_cosine_angle, COSINE_ANGLE_AT_MINIMUM_ELEMENT_LENGTH);
  r = (COSINE_ANGLE_AT_MINIMUM_ELEMENT_LENGTH - max_cosine_angle) /
      (COSINE_ANGLE_AT_MINIMUM_ELEMENT_LENGTH - (-1.0));

  return (1.0 - r) * dd->bc.element_length[0] + r * dd->bc.element_length[1];
  */
 return dd->bc.element_length[0];
}
...
static void ControlDislocationElementLength(DD_t *dd) {
  for (int iElement = 0; iElement < dd->nElements; iElement++) {
    ...
    if (length > 2.0 * standard_element_length) {
      // If the element length is too long, the element is divided into two
      ...
      // If the element length is too short, the element is removed

...
class Dislocations:
        ...
        with open(f"{selected_folder}/elem.inp", "w") as f:
            ...
            for i, point in enumerate(self.points):
                line = (
                    ...
                    + " ".join(f"{coord:.8e}" for coord in point)
                    + "\n"
                )
                f.write(line)
            ...
            for i, element in enumerate(self.elements_nodes):
                line = (
                    ...
                    + " "
                    + " ".join(f"{val:.8e}" for val in self.burgers_vectors[i])
                    + " "
                    + " ".join(f"{val:.8e}" for val in self.slip_planes[i])
                    + "\n"
                )
                f.write(line)
    ...
    def write_to_vtk(self, selected_folder):
        with open(f"{selected_folder}/elem.vtk", "w") as f:
            ...
            for i, point in enumerate(self.points):
                line = " ".join(f"{coord:.8e}" for coord in point) + "\n"
                f.write(line)
            ...
            for i, burgers_vector in enumerate(self.burgers_vectors):
                line = " ".join(f"{val:.8e}" for val in burgers_vector) + "\n"
                f.write(line)
            f.write("VECTORS SlipPlane float\n")
            for i, slip_plane in enumerate(self.slip_planes):
                line = " ".join(f"{val:.8e}" for val in slip_plane) + "\n"
                f.write(line)

...
def _type_label_modifier_factory(radius_map: Dict[int, Optional[float]]):
    ...
    def _apply(frame, data):
        ...
        _apply_to_type(1, "Si", (0.0, 70/255, 160/255))  # ブルー
        _apply_to_type(2, "C",  (1.0, 0.1, 0.1))   # 赤
...
def render_image(node, output_path: str, width: int, height: int, dpi: int,
    ...
    try:
        ...
        renderer.ambient_occlusion_radius = 6.0
        renderer.ambient_occlusion_samples = 128
    except:
        pass

    # ★ Principled Material の光沢最大化
    try:
        renderer.principled_base_color = (1.0, 1.0, 1.0)
        renderer.principled_metallic = 0.5      # 金属感
        renderer.principled_specular = 1.0      # 反射強度
        renderer.principled_specular_tint = 0.0
        renderer.principled_roughness = 0.05    # 非常に滑らか
        renderer.principled_anisotropy = 0.2
        renderer.principled_clearcoat = 1.0     # クリア層で光沢UP
        renderer.principled_clearcoat_gloss = 1.0
        renderer.principled_ior = 1.45          # 屈折率(ガラス程度)
        renderer.principled_transmission = 0.4  # 透明度は無し
    except:
        pass
...
def main():
    ...
    parser.add_argument("--input", type=str, default='/Users/kyou/Library/CloudStorage/Box-Box/output/4HSiC/vacancy/vashishta-k/disp/4hsic_q/3.sic',
    ...
    parser.add_argument("--radius-si", type=float, default=0.28,
                        help="Si の粒子半径（未指定なら --radius を利用）")
    parser.add_argument("--radius-c", type=float, default=0.18,
    ...
    parser.add_argument("--camera-dir", type=_vector3, default=(0, 0, -1.0),
                        help="視線方向（\"x,y,z\"）")
    parser.add_argument("--camera-pos", type=_vector3, default=(4, 4, 15.0),
                        help="カメラ位置（\"x,y,z\"）")

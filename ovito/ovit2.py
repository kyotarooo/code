...
def setup_visual(node, bond_width: float, specular: float):
    ...
    if pvis is not None:
        ...
        try:
            ...
            pass
        # 3.12.0では粒子タイプ半径を使うと立体感が出る
        try:
            pvis.use_particle_radii = True
        except Exception:
            pass
        try:
            pvis.radius = 0.0  # タイプ個別半径に完全依存
        except Exception:
            pass
        try:
...
def render_image(node, output_path: str, width: int, height: int, dpi: int,
    ...
    from ovito.vis import Viewport, OSPRayRenderer, OpenGLRenderer
    ...
    # --- ★ 影（シャドウ）をON（最重要） ---
    try:
        renderer.shadows_enabled = True   # 影 ON
        renderer.shadow_intensity = 0.8   # 濃い影
    except:
        pass

    # --- ★ AO（Ambient Occlusion）強化 ---
    try:
        renderer.ambient_occlusion_enabled = True
        renderer.ambient_occlusion_intensity = 1.2   # デフォルト0.6
        renderer.ambient_occlusion_radius = 3.0      # AOの広がり
        renderer.ambient_occlusion_samples = 128     # サンプル増加
    except:
        pass
    ...
    try:
        ...
        renderer.principled_specular_roughness = 0.1   # 表面をツルツルに
    except:
        pass
...
def main():
    ...
    parser.add_argument("--input", type=str, default='/Users/kyou/Library/CloudStorage/Box-Box/output/4HSiC/vacancy/vashishta-k/disp/4hsic_vacancy/atom_type_to_delete_0/1_in.sicq',
                        help="入力構造ファイル（LAMMPS data など）")
    parser.add_argument("--output", type=str, default="./render_4HSiC.png",
                        help="出力PNGパス")
    parser.add_argument("--cutoff", type=float, default=1.00,
                        help="結合生成のカットオフ [Å]")
    ...
    parser.add_argument("--bondwidth", type=float, default=0.10,
                        help="結合線の太さ")
    ...
    parser.add_argument("--camera-dir", type=_vector3, default=(-1.5, -1.0, -1.0),
                        help="視線方向（\"x,y,z\"）")
    parser.add_argument("--camera-pos", type=_vector3, default=(30, 20.0, 20.0),
                        help="カメラ位置（\"x,y,z\"）")
    parser.add_argument("--fov", type=float, default=35.0,
                        help="視野角[deg]")
    parser.add_argument("--specular", type=float, default=0.60,
                        help="球のハイライト強度")
    parser.add_argument("--shininess", type=float, default=0.6,

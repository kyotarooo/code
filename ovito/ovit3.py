...
def _type_label_modifier_factory(radius_map: Dict[int, Optional[float]]):
    ...
    def _apply(frame, data):
        ...
        _apply_to_type(1, "Si", (0.0, 0.65, 1.0, 0.4))   # Si → 青：40% 透明
        _apply_to_type(2, "C",  (0.50, 1.00, 0.83, 0.3)) # C → オレンジ：70% 透明
        _apply_to_type(3, "N",  (1.0, 0.10, 0.10, 0.5))  # N → 赤：50% 透明
...
def main():
    ...
    parser.add_argument("--radius-si", type=float, default=1.1)
    parser.add_argument("--radius-c",  type=float, default=0.7)
    parser.add_argument("--radius-t3", type=float, default=1.1, help="N 原子の半径（未指定なら Si と同じ）")

    # --- 結合距離 ---
    parser.add_argument("--cutoff-12", type=float, default=1.0)
    parser.add_argument("--cutoff-13", type=float, default=1.0)
    parser.add_argument("--cutoff-23", type=float, default=1.0)
    ...
    parser.add_argument("--camera-pos", type=_vector3, default=(30, 20, 20))

...
with open(f"/Users/kyou/Library/CloudStorage/Box-Box/lammps_related/{lammpsfilename}","w") as f:
    ...
    f.write('region Al_region block -10 10 -10 10 -10 10\n')
    f.write('create_box 1 Al_region\n')
    f.write('create_atoms 1 region Al_region\n')
    f.write('mass 1 26.98\n')
    f.write('pair_style eam/alloy\n')
    f.write('pair_coeff * * /Users/kyou/Library/CloudStorage/Box-Box/interatomic_potentials/Al99.eam.alloy Al\n')
    #******原子空孔領域******    
    f.write('region vacancyspot sphere 0 0.1 0 0.6\n')
    f.write('group vacancyatom region vacancyspot\n')
    #******force dipole の適用領域　立方体で定義********
    for m in cub_side_length:
        n = 2*m
...
#******原子空孔の情報を出力*******←dumpの番号を分ければ、undumpする必要ない？
    ...
    for i in range(1, 10):
        ...
        f.write(f'fix printstress{i} all print 1 "${{pxx{i}}} ${{pyy{i}}} ${{pzz{i}}} ${{pxy{i}}} ${{pxz{i}}} ${{pyz{i}}}" '
                f'file /Users/kyou/Library/CloudStorage/Box-Box/lammps_related/sumstress_Al/stress{i}.txt screen no\n')
    f.write('run 1\n')

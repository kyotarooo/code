...
static double AdaptiveElementLengthControl(int iElement, DD_t *dd) {
  ELEMENT_t *element = &dd->element[iElement];
  double max_cosine_angle = -2.0;
  double r;
  double smooth_angle, angle;

  for (int iNode = 0; iNode < 2; iNode++) {
    int nodeID = element->nodeID[iNode];
    double cosine_angle = DD_ElementCosineAngle(nodeID, dd);

    if (cosine_angle >= 1.5) continue;

    max_cosine_angle = fmax(cosine_angle, max_cosine_angle);
  }

  // Cosine function-based
  max_cosine_angle = fmin(max_cosine_angle, SMOOTH_COSINE_ANGLE);
  r = (SMOOTH_COSINE_ANGLE - max_cosine_angle) / (SMOOTH_COSINE_ANGLE - (-1.0));

  // Angle-based
  // smooth_angle = acos(SMOOTH_COSINE_ANGLE);
  // angle = acos(max_cosine_angle);
  // r = (angle - smooth_angle) / (M_PI - smooth_angle);
  
  return (1.0 - r) * dd->bc.element_length[0] + r * dd->bc.element_length[1];
}

static void ControlDislocationElementLength(DD_t *dd) {
  double min_element_length = dd->bc.element_length[0];
  double max_element_length = dd->bc.element_length[1];

  for (int iElement = 0; iElement < dd->nElements; iElement++) {
    double standard_element_length = AdaptiveElementLengthControl(iElement, dd);

...
void DD_DeleteElement(int elementID, DD_t *dd) {
  for (int iElement = elementID; iElement < dd->nElements - 1; iElement++) {
    ELEMENT_t *element0 = &dd->element[iElement];
    ELEMENT_t *element1 = &dd->element[iElement + 1];
...
void DD_ConnectElementPairs(int nodeID0, int nodeID1, DD_t *dd) {
  ...
  int nElement_pairs =
      ElementPairs(nodeID0, nodeID1, dd, element_pair);

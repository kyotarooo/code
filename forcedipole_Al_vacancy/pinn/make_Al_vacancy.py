...
######## supercell のサイズ ########
units_cell_x = [2, 4, 6, 8, 10, 12, 14, 16, 18]
units_cell_y = [2, 4, 6, 8, 10, 12, 14, 16, 18]
units_cell_z = [2, 4, 6, 8, 10, 12, 14, 16, 18]
atom_num = []
filename = []
...
for i in range(len(units_cell_x)):
    ...
    name = f"vacancy_Al_{num}.in"
    atom_num.append(num)
    filename.append(name)


######## ファイル名の出力 ########   ←これはshellscriptに
with open(f"/Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Al/displacement/infilename.txt", 'w') as f_num:
    for i in range(len(units_cell_x)):
        f_num.write(f"{filename[i]} ")

######## lammps script #########
for i in tqdm(range(len(units_cell_x))):
    with open(f"/Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Al/displacement/in_file/{filename[i]}", 'w') as f:
        f.write(f'log /Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Al/displacement/log_file/log_vacancy_Al_{atom_num[i]}.lammps\n')
        f.write('clear\n')
        f.write('units metal\n')
        ...
        # fcc格子
        f.write(f'lattice {lattice_type} {lattice_const}\n')
        ...
        f.write(f'print "${{N}}" append /Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Al/displacement/N_atom.txt screen no\n')

        #lattce sizeの出力
        f.write(f'variable ax equal xlat\n')
        f.write(f'variable ay equal ylat\n')
        f.write(f'variable az equal zlat\n')
        f.write(f'print "${{ax}} ${{ay}} ${{az}}" append /Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Al/displacement/lattice_size.txt screen no\n')

        #supercell sizeの出力
        f.write(f'variable lx equal lx\n')
        f.write(f'variable ly equal ly\n')
        f.write(f'variable lz equal lz\n')
        f.write(f'print "${{lx}} ${{ly}} ${{lz}}" append /Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Al/displacement/supercell_size.txt screen no\n')
        
        #potential(pinn)
        f.write(f'pair_style pinn\n')
        f.write(f'pair_coeff * * /Users/kyou/Library/CloudStorage/Box-Box/interatomic_potentials/Al_2020.pinn Al\n')
        ...
        # dump (deleted atom)
        f.write(f'dump deleted_atoms vacancy_atom{i} custom 1 /Users/kyou/Library/CloudStorage/Box-Box/dump/vacancy_Al/deleted_atom/deleted_atom{i}.dump id type x y z\n')
        f.write('dump_modify deleted_atoms format line "%d %d %.12e %.12e %.12e"\n')
        f.write('run 0\n')

        # 空孔の削除
        f.write(f'delete_atoms group vacancy_atom{i}\n')
        f.write(f'variable N equal count(all)\n')
        f.write(f'print "${{N}}" append /Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Al/displacement/N_atom_defect.txt screen no\n')
        f.write(f'run 0\n')

        # dump (perfect lattice)
        f.write(f'dump perfect all custom 1 /Users/kyou/Library/CloudStorage/Box-Box/dump/vacancy_Al/perfect_lattice/perfect_lattice{i}.dump id type x y z\n') 
        f.write('dump_modify perfect format line "%d %d %.12e %.12e %.12e"\n')
        f.write("run 0\n")
        ...
        # dump (defect lattice)
        f.write(f'dump defect all custom 1 /Users/kyou/Library/CloudStorage/Box-Box/dump/vacancy_Al/defect_lattice/defect_lattice{i}.dump id type x y z\n') 
        f.write('dump_modify defect format line "%d %d %.12e %.12e %.12e"\n')
        f.write("run 0\n")

...
print(f"{'lambda:':<10}{'Voigt average':<25}= {E_v / 1e9:10.3f} GPa")
print(f"{'lambda:':<10}{'Reuss average':<25} = {E_r / 1e9:10.3f} GPa")
print(f"{'mu: Voigt average':<21}     = {nu_v:10.3f} [-]")
print(f"{'mu: Reuss average':<21}     = {nu_r:10.3f} [-]")
...
######## ファイル数の読み込み ########
with open(f"/Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Al/displacement/infilename.txt", 'r') as f_num:
...
disp_threshold = []
supercell_size = []
with open(f"/Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Al/displacement/supercell_size.txt", 'r') as f_size:
...
for i in range(file_num):
    coordinate_p = read_perfect_data(f"/Users/kyou/Library/CloudStorage/Box-Box/dump/vacancy_Al/perfect_lattice/perfect_lattice{i}.dump")
    coordinate_of_perfect.append(coordinate_p)
    coordinate_d = read_defect_data(f"/Users/kyou/Library/CloudStorage/Box-Box/dump/vacancy_Al/defect_lattice/defect_lattice{i}.dump")
    coordinate_of_defect.append(coordinate_d)
    coordinate_v = read_vacancydata(f"/Users/kyou/Library/CloudStorage/Box-Box/dump/vacancy_Al/deleted_atom/deleted_atom{i}.dump")
...
######## displacementの出力 ########
for i in range(file_num):
    with open(f"/Users/kyou/Library/CloudStorage/Box-Box/dump/vacancy_Al/displacement/displacement{i}.inp", "w") as f_disp,\
        open(f"/Users/kyou/Library/CloudStorage/Box-Box/dump/vacancy_Al/displacement/displacement_ovit{i}.xyz", "w") as f_disp_ovit:

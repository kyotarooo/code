...
def main():
    ...
    for l in range(file_num):
       ...
       # ====== Compute radial displacement from MD ======
        ...
        # ====== Plot MD result ======
        plt.figure()
        plt.plot(abs_rx_list, ur_list, "o",mfc="#0075c2", markersize = 6, mec = "black", label = "$Ur$",  color="black")
        ...
        # --- make dir & save png --=
        save_dir = f"{output_dir}/MD_graph"
        os.makedirs(save_dir, exist_ok=True)
        plt.savefig(f"{save_dir}/MD_disp{n}", dpi=300)
        plt.close()
        ...
        for i in range(cutoff_num):
            for rx in sorted_mesh_atom_distance_list:
                ...
                if abs_rx < 1e-15:  
                    continue
                abs_rx_list_green[i].append(abs_rx)
                nr = rx / abs_rx
                green = np.zeros((3, 3, 3))
                green = kelvin_solution(g, v, rx, green)
                ux=uy=uz=0.0
                for j in range(9):
                    ux -= green[0][iidx[j]][jidx[j]]*P[i][1][j]
                    uy -= green[1][iidx[j]][jidx[j]]*P[i][1][j]
                    uz -= green[2][iidx[j]][jidx[j]]*P[i][1][j]
                ui = [ux, uy, uz]
                ur = (-1)*np.dot(nr, ui)
                ur_list_green[i].append(ur)
            abs_rx_list_green[i] = np.array(abs_rx_list_green[i]) / lattice_const
            ur_list_green[i] = np.array(ur_list_green[i]) / lattice_const

            # ====== Plot Kelvin result ======    
        plt.figure()
        for i in range(cutoff_num):
            if i == 3:
                plt.plot(abs_rx_list_green[i], ur_list_green[i], "-", label = "$ur$",  color="black",linewidth=2.0)
        plt.axhline(y=0.0, color='#808080', linestyle='--', linewidth=0.5)
        plt.xlabel(r"$\it{Distance} \,/\, a \, [-]$")
        plt.ylabel(r"$\it{Displacement} \,/\, a \, [-]$")
        plt.title(f"$\\it{{Number\\ of\\ atoms}}$ = {n}")
        ...
        # --- make dir & save png ---
        save_dir = f"{output_dir}/green_graph"
        os.makedirs(save_dir, exist_ok=True)
        plt.savefig(f"{save_dir}/green_disp{n}", dpi=300)
        plt.grid(True, color='gray', linestyle='--', linewidth=0.5)
        plt.legend(
        frameon=True,
        facecolor='white',
        ...
        # ############# Plot comparison: MD vs Kelvin(cut_off) ############
        ...
        for i in range(cutoff_num):
            plt.plot(abs_rx_list_green[i], ur_list_green[i], "-",label = label_list[i], color = color_list[i], linewidth=1.0)
        plt.axhline(y=0.0, color='#808080', linestyle='--', linewidth=0.5)
        plt.xlabel(r"$\it{Distance} \,/\, a \, [-]$")
        plt.ylabel(r"$\it{Displacement} \,/\, a \, [-]$")
        plt.title("$Displacement Method\\ Cut-off\\ radius\\ r_{excl}$")
        plt.title("$\\it{{Displacement\\ Method\\ (\\ Cut-off\\ Radius\\ :\\ r_{excl})}}$")
        plt.ylim([-0.001,0.014])
        plt.xlim(left = 0.0)
        plt.grid(True, color='gray', linestyle='--', linewidth=0.5)
        plt.legend(
        ...
        # --- make dir & save png ---
        save_dir = f"{output_dir}/MD_vs_green_graph"
        os.makedirs(save_dir, exist_ok=True)
        plt.savefig(f"{save_dir}/green_disp{n}", dpi=300)
        plt.close()

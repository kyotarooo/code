...
def main():
    ######### cut offの定義 ########
    x_list = []
    limit_range = np.array([0, 1, 1.5, 2]) # [-]

    for l in range(file_num):
        with open(f"{output_dir}/disp_data/MD_disp_data{l}.txt", "w") as f_MD_disp_data:
            for mi, m in enumerate(limit_range):
                r_range_limit = lattice_const * m * conv_ang_to_m
                a = np.zeros((9, 9))
                b = np.zeros(9)
                ...
                for xi,ui,rx in tqdm(sorted_atom_distance_list):
                    ######### MD データの書き出し ########
                    if l != 0:
                        f_MD_disp_data.write(f"{np.linalg.norm(rx)} {rx[0]} {rx[1]} {rx[2]} {ui[0]} {ui[1]} {ui[2]}\n")

                    if np.linalg.norm(rx)>r_range_limit:
                        green = np.zeros((3, 3, 3))
                        counter += 1
                        green, r = kelvin_solution(g, v, rx, green)
                        green_list.append((green, r, rx))
                ...
                ######### 理論解(ux_i = G_ij,k x p_j,k)の書き出し ########
                green_list = sorted(green_list, key=lambda x: x[1])
                if l != 0:
                    with open(f"{output_dir}/ux_data/ux_data{l}_{mi}", "w") as f_ux:
                        for (green, r, rx) in green_list:
                            ux=uy=uz=0
                            for j in range(9):
                                ux -= green[0][iidx[j]][jidx[j]]*x[j]
                                uy -= green[1][iidx[j]][jidx[j]]*x[j]
                                uz -= green[2][iidx[j]][jidx[j]]*x[j]
                            
                            f_ux.write(f"{r} {rx[0]} {rx[1]} {rx[2]} {ux} {uy} {uz}\n")

...
c_11 = c_11 * 1.0e+6 * 1.0e+5 #pa
c_12 = c_12 * 1.0e+6 * 1.0e+5 #pa
c_44 = c_44 * 1.0e+6 * 1.0e+5 #pa
...
disp_threshold = []
supercell_size = []
with open(f"{output_dir}/supercell_size.txt", 'r') as f_size:
...
for i in range(file_num):
    coordinate_p = read_perfect_data(f"{output_dir}/dump/perfect_lattice/perfect_lattice{i}.dump")
    coordinate_of_perfect.append(coordinate_p)
    coordinate_d = read_defect_data(f"{output_dir}/dump/defect_lattice/defect_lattice{i}.dump")
    coordinate_of_defect.append(coordinate_d)
    coordinate_v = read_vacancydata(f"{output_dir}/dump/deleted_atom/deleted_atom{i}.dump")
...
######## displacementの出力 ########
for i in range(file_num):
    with open(f"{output_dir}/dump/displacement/displacement{i}.inp", "w") as f_disp,\
        open(f"{output_dir}/dump/displacement_ovit/displacement_ovit{i}.xyz", "w") as f_disp_ovit:

...
def main():
    ...
    for l in range(file_num):
        ...
        rmse_disp = {i : [] for i in range(file_num)}
        x_disp = {i : [] for i in range(file_num)}
        ...
        for la in range(file_num):
            plt.plot(x_disp[la], rmse_disp[la], "-", label = label_list[la], linewidth = 1.0, color = colors[la])
        plt.plot(x_residual[6], rmse_residual[6], "-", label = "$\\it{Residual\\ Stress}$", linewidth = 1.0, color = "#0000ff")
        ...
        borderpad=0.3,        # ← 凡例枠内の余白（デフォルト: 0.4）
            ...
            # --- make dir & save png ---
        save_dir = f"{output_dir}/_disp_graph"
        os.makedirs(save_dir, exist_ok=True)
        plt.savefig(f"{save_dir}/rsme_disp{l}.png", dpi=300)
        plt.close()

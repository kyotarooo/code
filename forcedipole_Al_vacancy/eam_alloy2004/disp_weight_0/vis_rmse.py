...
with open(f"{output_dir}/filename.txt", 'r') as f_num:
...
with open(f"{dipole_r_path}/force_dipole_{atom_type_to_delete}/force_dipole_J.txt", "r") as f_r:
    lines = f_r.readlines()
    P_r = []
    for i in range(len(lines)):
        values = np.array([float(v) for v in lines[i].split()])
...
def main():
    ...
    with open(f"{output_dir}/dump/displacement/displacement_{atom_type_to_delete}/displacement{3}.inp", "r") as f_disp:   #←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←どの原子数でrmseをとるか
    ...
    for la in range(cutoff_num):
        plt.plot(atoms, rmse_disp[la], marker_list[la], label = label_list[la], linewidth = 1.0, color = "black",  markersize = 7,  mfc=color_list[la], ls = "-")
    plt.plot(atoms, rmse_residual, marker = "h", label=r"$\it{Residual\ Stress\ }$", linewidth = 1.0, mfc = "#ff0000", color = "black", ls = "-", markersize = 7)

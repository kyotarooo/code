...
def main():
    ...
    with open(f"{output_dir}/force_dipole/data_p_eV.inp", "r") as f_eV,\
        ...
        pxx_eV = {i: [] for i in range(5)}
        pyy_eV = {i: [] for i in range(5)}
        pzz_eV = {i: [] for i in range(5)}
        pxx_J = {i: [] for i in range(5)}
        pyy_J = {i: [] for i in range(5)}
        pzz_J = {i: [] for i in range(5)}
        atoms = []

        for i in range(n):
            mod = i % num_cutoff
            p1_eV, p2_eV, p3_eV = [float(v) if v.lower() != 'nan' else np.nan for v in lines_eV[i].split()[:3]]
            p1_J, p2_J, p3_J = [float(v) if v.lower() != 'nan' else np.nan for v in lines_J[i].split()[:3]]

...
def main():
    with open(f"{output_dir}/force_dipole/force_dipole_eV.txt", "r") as f_eV,\
         open(f"{output_dir}/force_dipole/force_dipole_J.txt", "r") as f_J,\
        ...
        for i in range(n):
            ...
            p2_eV = float(lines_eV[i].split()[4])
            p3_eV = float(lines_eV[i].split()[8])
            p1_J = float(lines_J[i].split()[0])
            p2_J = float(lines_J[i].split()[4])
            p3_J = float(lines_J[i].split()[8])
            pxx_eV.append(p1_eV)
            pyy_eV.append(p2_eV)
            pzz_eV.append(p3_eV)
            pxx_J.append(p1_J)

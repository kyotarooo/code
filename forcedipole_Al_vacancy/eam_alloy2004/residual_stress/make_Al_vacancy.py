...
for i in range(len(units_cell_x)):
    with open(f"{output_dir}/in_file/{filename[i]}", 'w') as f:
        f.write(f'log {output_dir}/log_file/log_{atom_num[i]}.lammps\n')
        f.write('clear\n')
        f.write('units metal\n')
        ...
        f.write(f'pair_coeff * * /Users/kyou/Library/CloudStorage/Box-Box/interatomic_potentials/Al-LEA.eam.alloy Al\n')
        ...
        f.write(f'thermo_style custom step c_pe_sumstress{i}[1] c_pe_sumstress{i}[2] c_pe_sumstress{i}[3] c_pe_sumstress{i}[4] c_pe_sumstress{i}[5] c_pe_sumstress{i}[6]\n')
        f.write('run 0\n')
        #*****
        ...
        f.write(f'print "{i} ${{pe_pxx_J}} ${{pe_pxy_J}} ${{pe_pxz_J}} ${{pe_pxy_J}} ${{pe_pyy_J}} ${{pe_pyz_J}} ${{pe_pxz_J}} ${{pe_pyz_J}} ${{pe_pzz_J}}" append {output_dir}/sumstress/perfect_J_sumstress.txt screen no\n')
        f.write('run 0\n')
        ...
        f.write(f'print "{i} ${{pe_pxx_eV}} ${{pe_pxy_eV}} ${{pe_pxz_eV}} ${{pe_pxy_eV}} ${{pe_pyy_eV}} ${{pe_pyz_eV}} ${{pe_pxz_eV}} ${{pe_pyz_eV}} ${{pe_pzz_eV}}" append {output_dir}/sumstress/perfect_eV_sumstress.txt screen no\n')
        f.write('run 0\n')
        ...
        f.write(f'thermo_style custom step c_de_sumstress{i}[1] c_de_sumstress{i}[2] c_de_sumstress{i}[3] c_de_sumstress{i}[4] c_de_sumstress{i}[5] c_de_sumstress{i}[6]\n')
        f.write('run 0\n')
        ...
        f.write(f'print "{i} ${{de_pxx_J}} ${{de_pxy_J}} ${{de_pxz_J}} ${{de_pxy_J}} ${{de_pyy_J}} ${{de_pyz_J}} ${{de_pxz_J}} ${{de_pyz_J}} ${{de_pzz_J}}" append {output_dir}/sumstress/defect_J_sumstress.txt screen no\n')
        f.write('run 0\n')
        ...
        f.write(f'print "{i} ${{de_pxx_eV}} ${{de_pxy_eV}} ${{de_pxz_eV}} ${{de_pxy_eV}} ${{de_pyy_eV}} ${{de_pyz_eV}} ${{de_pxz_eV}} ${{de_pyz_eV}} ${{de_pzz_eV}}" append {output_dir}/sumstress/defect_eV_sumstress.txt screen no\n')
        f.write('run 0\n')

...
######### 定数の定義 ############
# E = 88e+09  # Pa ←←←理科年表で調べる！！
# nu = 0.34 
# g = E / (2 * (1 + nu))  # 剛性率
# lattice_const = 3.22029900000000
# mass = 9.12240000000000E+0001
lattice_type = "hcp"
...
for i in range(len(units_cell_x)):
    ...
    name = f"vacancy_Zr_{num}.in"
    atom_num.append(num)
    filename.append(name)


######## ファイル名の出力 ########
with open(f"/Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Zr/infilename.txt", 'w') as f_num:
    for i in range(len(units_cell_x)):
...
for i in tqdm(range(len(units_cell_x))):
    with open(f"/Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Zr/in_file/{filename[i]}", 'w') as f:
        f.write(f'log /Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Zr/log_file/log_vacancy_Zr_{atom_num[i]}.lammps\n')
        f.write('clear\n')
        f.write('units metal\n')
        ...
        # HCP格子
        f.write('lattice custom 3.234 a1 1.0 0.0 0.0 a2 -0.5 0.8660254 0.0 a3 0.0 0.0 1.598 '
        ...
        f.write(f'print "${{N}}" append /Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Zr/N_atom.txt screen no\n')

        #lattce sizeの出力
        f.write(f'variable ax equal xlat\n')
        f.write(f'variable ay equal ylat\n')
        f.write(f'variable az equal zlat\n')
        f.write(f'print "${{ax}} ${{ay}} ${{az}}" append /Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Zr/lattice_size.txt screen no\n')

        #supercell sizeの出力
        f.write(f'variable lx equal lx\n')
        f.write(f'variable ly equal ly\n')
        f.write(f'variable lz equal lz\n')
        f.write(f'print "${{lx}} ${{ly}} ${{lz}}" append /Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Zr/supercell_size.txt screen no\n')
        ...
        # dump (deleted atom)
        f.write(f'dump deleted_atoms vacancy_atom{i} custom 1 /Users/kyou/Library/CloudStorage/Box-Box/dump/vacancy_Zr/deleted_atom/deleted_atom{i}.dump id type x y z\n')
        ...
        f.write(f'variable N equal count(all)\n')
        f.write(f'print "${{N}}" append /Users/kyou/Library/CloudStorage/Box-Box/lammps/pointdefects_Zr/N_atom_defect.txt screen no\n')
        f.write(f'run 0\n')

        # dump (perfect lattice)
        f.write(f'dump perfect all custom 1 /Users/kyou/Library/CloudStorage/Box-Box/dump/vacancy_Zr/perfect_lattice/perfect_lattice{i}.dump id type x y z\n') 
        ...
        # dump (defect lattice)
        f.write(f'dump defect all custom 1 /Users/kyou/Library/CloudStorage/Box-Box/dump/vacancy_Zr/defect_lattice/defect_lattice{i}.dump id type x y z\n') 

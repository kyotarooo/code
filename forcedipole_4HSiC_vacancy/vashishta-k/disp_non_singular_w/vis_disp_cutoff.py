...
def main():
    for l in range(file_num):
        ...
        abs_rx_list_green = {i: [] for i in range(cutoff_num)}
        ur_list_green = {i: [] for i in range(cutoff_num)}
        ...
        for cutoff in range(cutoff_num):
            for rx in sorted_mesh_atom_distance_list:
                ...
                if abs_rx == 0:  
                    continue
                abs_rx_list_green[cutoff].append(abs_rx)
                nr = rx / abs_rx
                ...
                for j in range(9):
                    ux_1_d -= green_1_d[0][iidx[j]][jidx[j]]*P[cutoff][5][j]    # ←←←←←←←←←←←←←←←←←←N = 959
                    uy_1_d -= green_1_d[1][iidx[j]][jidx[j]]*P[cutoff][5][j]
                    uz_1_d -= green_1_d[2][iidx[j]][jidx[j]]*P[cutoff][5][j]
                ...
                for j in range(9):
                    ux_2_d -= green_2_d[0][iidx[j]][jidx[j]]*P[cutoff][5][j]
                    uy_2_d -= green_2_d[1][iidx[j]][jidx[j]]*P[cutoff][5][j]
                    uz_2_d -= green_2_d[2][iidx[j]][jidx[j]]*P[cutoff][5][j]
                ui_2 = np.array([ux_2_d, uy_2_d, uz_2_d])
                ui = (1-m_weight) * ui_1 + m_weight * ui_2
                # -------------------------------------------------
                
                ur = (-1)*np.dot(nr, ui)
                ur_list_green[cutoff].append(ur)
            abs_rx_list_green[cutoff] = np.array(abs_rx_list_green[cutoff]) / lattice_const
            ur_list_green[cutoff] = np.array(ur_list_green[cutoff]) / lattice_const
        ...
        for cutoff in range(cutoff_num):
            plt.plot(abs_rx_list_green[cutoff], ur_list_green[cutoff], "-",label = label_list[cutoff], linewidth=1.0)
        ...
        # --- make dir & save png ---
        save_dir = f"{output_dir}/MD_disp_cutoff(959)/atom_type_to_delete_{atom_type_to_delete}/core_width_{a_core_width}"
        ...
        plt.rcParams["mathtext.fontset"] = "stix"  # STIXフォントはTimes系
        plt.rcParams["font.family"] = "STIXGeneral"
        colors = plt.cm.Blues(np.linspace(0.2, 1.0, cutoff_num))
        plt.plot(abs_rx_list, ur_list, "o",mfc="#ff0000", markersize = 2.5, mec = "black", label = "$Ur\\ (MD)$",  color="black", mew=0.5)
        for cutoff in range(cutoff_num):
            plt.plot(abs_rx_list_green[cutoff], ur_list_green[cutoff], "-",label = label_list[cutoff], linewidth=1.0, color=colors[cutoff])
        plt.plot(abs_rx_list_residual[5], ur_list_residual[5], "-",label = "$\\it{Residual\\ Stress}$", linewidth=1.0, color = "#ff0000")
        plt.axhline(y=0.0, color='#808080', linestyle='--', linewidth=0.5)
        ...
        # --- make dir & save png ---
        save_dir = f"{output_dir}/MD_disp_residual_cutoff(959)/MD_disp_residual_{atom_type_to_delete}/core_width_{a_core_width}"
        os.makedirs(save_dir, exist_ok=True)
        plt.savefig(f"{save_dir}/green_disp{atoms[l]}.png", dpi=300)
        plt.close()

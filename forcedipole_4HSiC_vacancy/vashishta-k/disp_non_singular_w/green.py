                ...
                if np.linalg.norm(rx)>r_range_limit:
                    ...
                    for i in range(9):
                        for j in range(9):
                            for k in range(3):
                                a[i][j] += (
                                    r * r * (((1-m_weight) * green1[k][iidx[i]][jidx[i]] + m_weight * green2[k][iidx[i]][jidx[i]]) * ((1-m_weight) * green1[k][iidx[j]][jidx[j]] + m_weight * green2[k][iidx[j]][jidx[j]]))
                                )

                    for i in range(9):
                        for j in range(3):
                            b[i] -= r * r * ((1-m_weight) * ui[j] * green1[j][iidx[i]][jidx[i]] + m_weight * ui[j] * green2[j][iidx[i]][jidx[i]])

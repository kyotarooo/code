...
with open(f"{dipole_r_path}/force_dipole/force_dipole_{atom_type_to_delete}/force_dipole_eV.txt", "r") as f_r:
    lines = f_r.readlines()
    P_r = []
...
def main():
    ...
    with open(f"{output_dir}/force_dipole/force_dipole_{atom_type_to_delete}/a_{a_core_width}/data_p_eV.inp", "r") as f_eV,\
         open(f"{output_dir}/force_dipole/force_dipole_{atom_type_to_delete}/a_{a_core_width}/data_p_J.inp", "r") as f_J,\
         open(f"{dipole_r_path}/force_dipole/force_dipole_{atom_type_to_delete}/force_dipole_eV.txt", "r") as f_residual,\
    ...
    for la in range(len(r_cutoff)):
            plt.plot(atoms[2:], pxx_eV[la][2:], marker_list[la], label = label_list[la], linewidth = 1.0, color = "black", markersize = 8, mfc=color_list[la], ls = "-")
    ...
    #plt.ylim(top=-2.0)
    # --- make dir & save png ---
    save_dir = f"{output_dir}/dipole_graph/delete_{atom_type_to_delete}/a_{a_core_width}"

...
def load_existing_file(file_name):
    try:
        ...
        values = {
            ...
            "ELEMENT_LENGTH": data_lines[14].split()[0],
        }
        return values
    except FileNotFoundError:
        eg.popup("No existing condition.inp file found. Using default values.")
        return defaults


# Initial GUI window for file selection
layout = [
    [eg.Text("Do you want to load an existing condition.inp file?")],
    [eg.Button("Yes"), eg.Button("No")],
]
window = eg.Window(
...
while window.is_alive():
    ...
    if event == "Yes":
        ...
        values = load_existing_file(selected_file)
        break
    elif event == "No":
        values = defaults
        break
...
layout = [
    [eg.Text("Time Step Configurations", background_color="gray", text_color="white")],
    [eg.Text("Number of time steps"), eg.Input(values["NUM_STEPS"], key="-NUM_STEPS-")],
    [
        eg.Text("Maximum displacement for each time step (m)"),
        eg.Input(values["UMAX"], key="-UMAX-"),
    ],
    [eg.Text("Minimum time step size (s)"), eg.Input(values["DTMIN"], key="-DTMIN-")],
    [eg.Text("Maximum time step size (s)"), eg.Input(values["DTMAX"], key="-DTMAX-")],
    [
        eg.Text("Output interval"),
        eg.Input(values["OUTPUT_INTERVAL"], key="-OUTPUT_INTERVAL-"),
    ],
    [eg.HSeparator()],
    [eg.Text("Material Properties", background_color="gray", text_color="white")],
    [
    ...
    [eg.Text("Mobility (/Ps s)"), eg.Input(values["MOBILITY"], key="-MOBILITY-")],
    [
        eg.Text("Dislocation core radius (m)"),
        eg.Input(values["CORE_RADIUS"], key="-CORE_RADIUS-"),
    ],
    [eg.HSeparator()],
    [eg.Text("Boundary Conditions", background_color="gray", text_color="white")],
    ...
    [eg.HSeparator()],
    [eg.Text("Simulation Parameters", background_color="gray", text_color="white")],
    [
        eg.Text("Standard element length (m)"),
        eg.Input(values["ELEMENT_LENGTH"], key="-ELEMENT_LENGTH-"),
    ],
    [eg.HSeparator()],
...
while window.is_alive():
    ...
    if event == "Save":
        ...
        with open(f"{selected_dir}/condition.inp", "w") as f:
            ...
            f.write(f"{values["-ELEMENT_LENGTH-"]}\n")
    ...
    if event == "Cancel":
        window.close()
        eg.popup("condition.inp not generated.")
        break
...
static void ModifyCoordinateInBoundaryLayer(double rx[3], double c, double b,
                                            double thickness) {
  double rd, rz;
...
static void AxialCylindricalInclusionStress(double x[3], double c, double b,
  ...
  ModifyCoordinateInBoundaryLayer(rx, c, b, thickness);
  rd = INCSTR_VectorLength(2, rx);
...
void INCSTR_AxialCylindricalInclusionStress(double x[3], int id,
  ...
  for (int i = 0; i < material->nImages; i++) {
    ...
    AxialCylindricalInclusionStress(x, c, b, thickness, ma, e, material, inside,
                                    local_stress);
  }
